...
namespace Lab9 {
    ...
    operation intToQubits (num : Int, register: Qubit[]) : Unit {
        
        //let numBits = Ceiling(Lg(IntAsDouble(num+1)));
        let boolArr = IntAsBoolArray(num, Length(register));

        //use register = Qubit[Length(boolArr)];
    ...
    operation fullAdder (a: Qubit, b: Qubit, cin: Qubit, sum: Qubit, cout: Qubit) : Unit{
        CNOT(a, sum);
        CNOT(b, sum);
        Controlled X([a,b], (cout));
        Controlled X([sum, cin], (cout));
        CNOT(cin, sum);

    }

    operation adder(num1: Qubit[], num2: Qubit[], result: Qubit[], carry: Qubit[]): Unit{
        // num1 and num2 registers must be the same size.
        // result register must be twice the size of num1/num2
        // apply full adders to each set of qubits

        // if (Length(num1) == 0 or Length(num2) == 0){
        //     return Unit;
        // }

        use cin = Qubit();

        fullAdder(num1[0], num2[0], cin, result[0], carry[0]);

        for i in 1..Length(num1)-1{
            fullAdder(num1[i], num2[i], carry[i-1], result[i], carry[i]);
        }
